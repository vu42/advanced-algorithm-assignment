...
- [Background and Related Work](#background-and-related-work)
  * [Boustrophedon Cellular Decomposition as an offline baseline](#boustrophedon-cellular-decomposition-as-an-offline-baseline)
  * [A* as used inside BA*](#a-as-used-inside-ba)
- [BA* Method](#ba-method)
  * [Workspace and robot model](#workspace-and-robot-model)
    + [Example: Phase 1 (Movement)](#example-phase-1-movement)
  * [Discrete tiling model $M$](#discrete-tiling-model-m)
    + [Tile definition and incremental construction](#tile-definition-and-incremental-construction)
      - [Explanation of $C_{free}$](#explanation-of-c_free)
    + [Covered vs visited and a consistent discovered state](#covered-vs-visited-and-a-consistent-discovered-state)
    + [Example: Phase 2 (Tiling)](#example-phase-2-tiling)
  * [Boustrophedon motion as coverage mode](#boustrophedon-motion-as-coverage-mode)
    + [Boustrophedon Motion and Critical Points](#boustrophedon-motion-and-critical-points)
    + [Critical point definition](#critical-point-definition)
    + [Example: Phase 3 (Stuck)](#example-phase-3-stuck)
  * [Backtracking list $L$](#backtracking-list-l)
    + [Neighbor indexing](#neighbor-indexing)
    + [Boundary indicator $b$](#boundary-indicator-b)
    + [Corner detector $\mu(s)$](#corner-detector-mus)
    + [List Construction](#list-construction)
    + [Example: Phase 4 (List Check)](#example-phase-4-list-check)
  * [Selection of the next starting point](#selection-of-the-next-starting-point)
    + [Example: Phase 5 (Selection)](#example-phase-5-selection)
  * [Backtracking path planning](#backtracking-path-planning)
    + [A* on the tiling graph and Smoothing](#a-on-the-tiling-graph-and-smoothing)
    + [Following the backtracking path](#following-the-backtracking-path)
      - [Explanation of parameters](#explanation-of-parameters)
      - [Semantic translation](#semantic-translation)
    + [Example: Phase 6 (Execution)](#example-phase-6-execution)
  * [Full BA* loop - Pseudocode](#full-ba-loop---pseudocode)
  * [Implementation Details](#implementation-details)
    + [Workspace representation and coordinates](#workspace-representation-and-coordinates)
    + [Discovered map state \hat{M} and tile states](#discovered-map-state-hatm-and-tile-states)
    + [Sensing and obstacle discovery](#sensing-and-obstacle-discovery)
    + [Blocked and uncovered free predicates](#blocked-and-uncovered-free-predicates)
    + [Boustrophedon motion mode used in code](#boustrophedon-motion-mode-used-in-code)
    + [Critical point test](#critical-point-test)
    + [Candidate list L construction via μ(s)](#candidate-list-l-construction-via-%CE%BCs)
    + [Backtracking graph and A*](#backtracking-graph-and-a)
    + [Selecting the next starting point s_sp](#selecting-the-next-starting-point-s_sp)
    + [A*SPT like smoothing and line of sight rule](#aspt-like-smoothing-and-line-of-sight-rule)
    + [Execution model and trajectory logging](#execution-model-and-trajectory-logging)
    + [Heading adjustment after backtracking](#heading-adjustment-after-backtracking)
    + [Obstacle inflation option](#obstacle-inflation-option)
    + [Reproducibility and outputs for the scenario](#reproducibility-and-outputs-for-the-scenario)
